...
public class AnalysisHelper {
    ...
    for (Comment c : data.getComments().values()) {
        ...
        commentNumbers.put(userId, commentNumbers.getOrDefault(userId, 0) + 1);
    }
    ...
    for (int i = 0; i < 5 && i < users.size(); i++) {
      ...
      public void getPassiveAndActiveoverallUsers () {
            ...
            System.out.println("Q7 - The following users have the least Comments: ");

            // Print users with the least overall activity
            for (int i = users.size() - 1; i >= 0 && i >= users.size() - 6; i--) {
                System.out.println(users.get(i) + ", - Overall count: " + overallNumbers.get(users.get(i).getId()));
            }
              }
      }
